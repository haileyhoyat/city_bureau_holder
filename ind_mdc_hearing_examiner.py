...
class IndMdcHearingExaminerSpider(CityScrapersSpider):
    name = "ind_mdc_hearing_examiner"
    agency = "Indianapolis MDC Hearing Examiner"
    timezone = "America/Chicago"
    start_urls = ["https://calendar.indy.gov/event/mdc-hearing-examiner-24/"]

    def parse(self, response):
        print("inside parse_result")
        meeting_list = []
        driver = webdriver.Chrome()
        driver.get("https://calendar.indy.gov/event/mdc-hearing-examiner-24/")  # noqa
        time.sleep(10)
        try:
